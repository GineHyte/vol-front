...
export default class Model {
	...
	get __tableData(): { [index: string]: any } {
		if (this.__pTableData.length > 0) {
			return this.__pTableData;
		}
		...
		this.__fields.forEach((dkey) => {
			...
			field.tableAlias.forEach((alias) => {
				value = value[alias];
			});
			...
			if (Array.isArray(data[key])) {
				if (Array.isArray(field.datatype)) {
					...
					this[dkey as keyof this] = data[key].map((item: any) => {
						const constructor = (field.datatype as { new (): Model }[])[0] as {
							new (): Model;
						};
	...
	getHeaders(exclude: string[] = []): { [key: string]: any }[] {
		return Object.keys(this.__deserializationMap)
			.map((dkey) => {
	...
	getCreationArray(exclude: string[] = []): { [key: string]: any }[] {
		if (this.__pCreationArray.length > 0) {
			return this.__pCreationArray;
		}
		let data: { [key: string]: any }[] = [];
		this.__fields.forEach((dkey) => {
			if (exclude.includes(dkey)) {
				return;
			}
			let _dkey = ('__' + dkey) as keyof this;
			const field = this[_dkey] as Field;
